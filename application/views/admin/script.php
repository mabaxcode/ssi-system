            ...
            (i = document.querySelector("#kt_modal_new_address")) && (o = new bootstrap.Modal(i), a = document.querySelector("#kt_modal_new_address_form"), t = document.getElementById("kt_modal_new_address_submit"), e = document.getElementById("kt_modal_new_address_cancel"), new Tagify(a.querySelector('[name="tags"]'), {
                ...
                e.preventDefault(), n && n.validate().then((function(e) {
                    console.log("validated!"), "Valid" == e ? (t.setAttribute("data-kt-indicator", "on"), t.disabled = !0, setTimeout((function() {
                        ...
                        $.ajax({
                            url: base_url + 'admin/create_inventory',
                            type: "POST",
                            data: workshopForm,
                            dataType: "json",
                            async: true,
...
$(document).on('click', '.edit-staff', function(e){
    ...
    e.preventDefault();
    $.ajax({
        url: base_url + 'admin/edit_inventory',
        type: "POST",
        data: {id:id},
        async: true,
        success: function( response ){
...
$(document).on('click', '.btn-edit-staff', function (e) {
    ...
    validateStaffEdit.validate().then(function(status) {
        ...
        if (status == 'Valid') {
            ...
            $.ajax({
                url: base_url + 'admin/do_edit_inventory',
                type: "POST",
            ...
            (i = document.querySelector("#kt_modal_new_vendor")) && (o = new bootstrap.Modal(i), a = document.querySelector("#kt_modal_new_address_form_vendor"), t = document.getElementById("kt_modal_new_address_submit_vendor"), e = document.getElementById("kt_modal_new_address_cancel_vendor"), new Tagify(a.querySelector('[name="tags"]'), {
                ...
                e.preventDefault(), n && n.validate().then((function(e) {
                    console.log("validated!"), "Valid" == e ? (t.setAttribute("data-kt-indicator", "on"), t.disabled = !0, setTimeout((function() {
                        ...
                        // doRegister(workshopForm);
                        $.ajax({
                            url: base_url + 'admin/create_user_account',
                            type: "POST",
                            data: workshopForm,
                            dataType: "json",
                            async: true,
...
$(document).on('click', '.delete-inventory', function(e){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
            url: base_url + 'admin/delete_inventory',
            type: "POST",
            data: {id:id},
            async: true,
            dataType:"json",
...
$(document).on('click', '.supply-item', function(e){
    var id = $(this).data('init');
    e.preventDefault();
    Swal.fire({
      // title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        $.ajax({
            url: base_url + 'admin/approve_item',
            type: "POST",
            data: {id:id},
            async: true,
            dataType:"json",
...
$(document).on('click', '.process-stock', function(e){
    var id = $(this).data('init');
    e.preventDefault();
    $.ajax({
        url: base_url + 'admin/process_stock',
        type: "POST",
        data: {id:id},
        async: true,
        success: function( response ){
...
<script type="text/javascript">
var validateStockComplete;
function completeStaffRegisStock(formID)
{   
    const fileForm = document.getElementById(formID);
    validateStockComplete = FormValidation.formValidation(
    fileForm,
    {
        fields: {
            approve_amount: {
                validators: {
                    notEmpty: {
                        message: 'Restock amount is required',
                    }
                }
            },
        },
...
$(document).on('click', '.btn-process-stock', function (e) {
    e.preventDefault();
    validateStockComplete.validate().then(function(status) {

        if (status == 'Valid') {

            var newStaffFormDataStock = $('#complete_staff_regis_data_stock').serialize();

            $.ajax({
                url: base_url + 'admin/do_restock',
                type: "POST",
                data: newStaffFormDataStock,
                dataType: "json",
                async: true,
                success: function( response ) {
                    // console.log(response);
